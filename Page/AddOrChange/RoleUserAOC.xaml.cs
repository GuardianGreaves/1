...
{
    public partial class RoleUserAOC : System.Windows.Controls.Page
    {        
        ...
        public RoleUserAOC(DataRowView row = null)
        {
            InitializeComponent();
            ...
            {
                ChangeOrAdd = false;
                NamePage.Text = "Добавление записи в таблицу \"Роль пользователя\"";
            }
        }
        ...
        private void SetCurrentRow(DataRowView row)
        {
            ...
            TextBoxAccess.Text = currentRow["Права_Доступа"].ToString();
            TextBoxDescription.Text = currentRow["Описание"].ToString();
            NamePage.Text = $"Редактирование записи №{Convert.ToInt32(currentRow["ID_Роли"])}";
        }
        ...
        private bool ExistsDuplicate()
        {
            var existingRecords = adapter.GetData()
                    .Where(row => row.Название == TextBoxName.Text && row.Права_Доступа == TextBoxAccess.Text && row.Описание == TextBoxDescription.Text);

            // При редактировании исключаем текущую запись по ее уникальному идентификатору, если он есть
            if (ChangeOrAdd && currentRow != null)
            {
                existingRecords = existingRecords.Where(row => row.ID_Роли != Convert.ToInt32(currentRow["ID_Роли"]));
            }
        ...
        private void BtnSave(object sender, RoutedEventArgs e)
        {
            ...
            string access = TextBoxAccess.Text;
            ...
            try
            {
                if (ChangeOrAdd)
                {
                    ...
                    currentRow["Права_Доступа"] = access;
                    currentRow["Описание"] = description;
                    // Сохраняем изменения в базе
                    adapter.Update((DP_2025_LoskutovaDataSet.РОЛЬDataTable)currentRow.DataView.Table);
                }
                else
                {
                    // Создаем новую запись в таблице
                    var newRow = db.РОЛЬ.NewРОЛЬRow();
                    newRow.Название = name;
                    newRow.Права_Доступа = access;
                    newRow.Описание = description;
                    db.РОЛЬ.Rows.Add(newRow);
